...
public class CustomerController {
    ...
    @PostMapping("/assign-rental/{customerId}")
    public ResponseEntity<String> assignRentalToCustomer(@PathVariable Long customerId, @RequestBody RentalRequest rentalRequest) {
        try {
            return ResponseEntity.ok(customerService.assignRentalToCustomer(customerId, rentalRequest));
        } catch (EntityNotFoundException e) {
            return  new ResponseEntity<>(e.getMessage(),HttpStatus.NOT_FOUND);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(),HttpStatus.INTERNAL_SERVER_ERROR);
